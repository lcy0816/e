...
int main()
{
    ...
    //     g[i] = 
    // }
    unsigned long long int *r = new unsigned long long int[4];//save all the sub keys 
    ...
    g[1] = (w^k) & 0xff00000000000000;
    g[1] >>= 56;//R0 first half 

    g[2] = (w^k) & 0xff000000000000;
    g[2] >>= 48;//R0 second half
    ...
    g[3] = ftable[g[2] ^ (int) ary[0][0]] ^ g[1];
    g[4] = ftable[g[3] ^ (int) ary[0][1]] ^ g[2];
    g[5] = ftable[g[4] ^ (int) ary[0][2]] ^ g[3];
    g[6] = ftable[g[5] ^ (int) ary[0][3]] ^ g[4];
    ...
    // unsigned int temp = g[2] ^ (int) ary[0][0];
    // cout << (ftable[g[2] ^ (int) ary[0][0]] ^ g[1]) << endl;
    g[9] = ftable[g[8] ^ (int) ary[0][4]] ^ g[7];
    g[10] = ftable[g[9] ^ (int) ary[0][5]] ^ g[8];
    g[11] = ftable[g[10] ^ (int) ary[0][6]] ^ g[9];
    g[12] = ftable[g[11] ^ (int) ary[0][7]] ^ g[10];
    ...
    f[0] = (t[0] + 0x2 * t[1] + (ary[0][8] << 8) + ary[0][9]) % 0x10000;
    cout << "f0: " << f[0] << " ";

    // cout << "This is the test key " << hex << (ary[0][10] << 8) + ary[0][11] << endl;
    f[1] = (0x2 * t[0] + t[1] + (ary[0][10] << 8) + ary[0][11]) % 0x10000;
