...
int main()
{
    ...
    char * key1 = new char[SIZE];// save the key from the file
    char * key2 = new char[SIZE];// save the key from the file
    // unsigned char * key_temp = new unsigned char[SIZE];// save the key from the file

    unsigned long long int k1 = 0;// the converted key format
    unsigned long long int k2 = 0;// the converted key format
    ...
    sprintf(key1, "0x%x%x%x%x", key[0], key[1], key[2], key[3]);
    sprintf(key2, "0x%x%x%x%x%x%x%x%x", key[4], key[5], key[6], key[7], key[8], key[9], key[10], key[11]);

    k1 = stoull(key1, nullptr, 0);
    k2 = stoull(key2, nullptr, 0);
    ...
    for (int j = 0; j < ROUND; ++j){
        ...
        for (int i = 0; i < SUB_KEY_LENGTH; ++i) {
            ...
            if ((j * 4 + add_on) % 8 == 3 ){
                ary[j][i] = k2 & 0xff000000;
                ary[j][i] >>= 24;
            }
            if ((j * 4 + add_on) % 8 == 4 ){
                ary[j][i] = k2 & 0xff00000000;
                ary[j][i] >>= 32;
            }
            if ((j * 4 + add_on) % 8 == 5 ){
                ary[j][i] = k2 & 0xff0000000000;
                ary[j][i] >>= 40;
            }
            if ((j * 4 + add_on) % 8 == 6 ){
                ary[j][i] = k2 & 0xff000000000000;
                ary[j][i] >>= 48;
            }
            if ((j * 4 + add_on) % 8 == 7 ){
                // if (to_be_msb == 0)
                    ary[j][i] = k2 & 0xff00000000000000;
    ...
    // }
    cout << "This is w^k: "<< hex << (w^k2) << endl;

    r[0] = (w^k2) & 0xffff000000000000;
    r[0] >>= 48;

    cout << "This is r0: " << r[0] << endl;
    r[1] = (w^k2) & 0xffff00000000;
    r[1] >>= 32;
    ...
    // cout << r[2] << endl;
    c[0] = r[2] ^ (k2 >> 48);
    c[1] = r[3] ^ (k2 & 0xffff00000000) >> 32;
    c[2] = r[0] ^ (k2 & 0xffff0000) >> 16;
    c[3] = r[1] ^ (k2 & 0xffff);
    cout << "Ciphertext: ";
    for (int i = 0; i < 4; ++i){
        cout << c[i];
    ...
    r[0] ^= (k2 >> 48);
    r[1] ^= (k2 & 0xffff00000000) >> 32;
    r[2] ^= (k2 & 0xffff0000) >> 16;
    r[3] ^= (k2 & 0xffff);
